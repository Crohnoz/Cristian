(() => {
  ...
  function starterCourse() {
    return {
      ...
      modules: [{
        id: uid('module'), title: 'Authorization Essentials', objective: 'Distinguir autenticación, autorización y control a nivel de objeto antes de entrar al laboratorio.',
        lessons: [
          { id: uid('lesson'), title: 'AuthN vs AuthZ', type: 'lesson', duration: 8, body: 'Conceptos esenciales y errores comunes al decidir quién puede hacer qué.' },
          { id: uid('lesson'), title: 'BOLA: lectura de evidencia', type: 'lab', duration: 18, body: 'Escenario aislado para identificar autorización insuficiente a nivel de objeto sin objetivos externos.' }
        ]
      }]
    };
  }
  ...
  function persist(reason = 'manual') {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ version: 1, tenantId, courses: state.courses }));
  ...
  function renderCatalog() {
    ...
    state.courses.forEach(course => {
      ...
      row.append(node('strong', course.title), node('span', `${course.modules.length} módulos · ${courseMinutes(course)} min · ${course.workflow.toUpperCase()}`));
  ...
  function renderCourseEditor() {
    ...
    controls.forEach(id => { if ($(id)) $(id).disabled = !course; });
    if (!course) {
    ...
    $('courseStatusChip').textContent = course.workflow.toUpperCase();
    $('courseMeta').textContent = `Actualizado ${new Date(course.updatedAt).toLocaleString('es-CL')} · ${course.modules.length} módulos · ${courseMinutes(course)} min`;
  }
  ...
  function renderLessons() {
    ...
    module.lessons.forEach((lesson, index) => {
      ...
      const remove = node('button', 'Eliminar', 'danger-quiet'); remove.type = 'button';
  ...
  function renderPreview() {
    const course = currentCourse();
    $('previewStatus').textContent = course?.workflow?.toUpperCase() || 'DRAFT';
    ...
    course.modules.forEach((module, index) => {
      ...
      const head = node('div'); head.append(node('strong', `${String(index + 1).padStart(2, '0')} · ${module.title}`), node('span', module.objective || 'Objetivo pendiente')); section.appendChild(head);
  ...
  function touch(course = currentCourse()) { if (course) course.updatedAt = now(); scheduleSave(); renderPreview(); }
  ...
  function deleteCourse() {
    ...
    if (!confirm(`Eliminar el borrador “${course.title}”? Esta acción solo afecta el almacenamiento local de esta rama.`)) return;
  ...
  function addModule() {
    const course = currentCourse(); if (!course || course.modules.length >= MAX_MODULES) return;
  ...
  function deleteModule() {
    const course = currentCourse(), module = currentModule(); if (!course || !module) return;
    course.modules = course.modules.filter(item => item.id !== module.id); selectedModuleId = course.modules[0]?.id || null; touch(course); renderAll(); showToast('Módulo eliminado.');
  }

  function addLesson() {
    const module = currentModule(), course = currentCourse(); if (!module || !course || module.lessons.length >= MAX_LESSONS) return;
  ...
  function removeLesson(id) {
    const module = currentModule(), course = currentCourse(); if (!module || !course) return;
  ...
  function updateLesson(id, patch) {
    const module = currentModule(), course = currentCourse(); const lesson = module?.lessons.find(item => item.id === id); if (!lesson || !course) return;
  ...
  function bindCourseFields() {
    $('courseTitle').addEventListener('input', event => { const course = currentCourse(); if (!course) return; const previous = course.slug; course.title = text(event.target.value, 120); if (!previous || previous.startsWith('nuevo-curso-')) { course.slug = slugify(course.title); $('courseSlug').value = course.slug; } touch(course); renderCatalog(); });
    $('courseSlug').addEventListener('input', event => { const course = currentCourse(); if (!course) return; course.slug = slugify(event.target.value); event.target.value = course.slug; touch(course); });
    $('courseSummary').addEventListener('input', event => { const course = currentCourse(); if (!course) return; course.summary = text(event.target.value, 700); touch(course); });
    $('courseLevel').addEventListener('change', event => { const course = currentCourse(); if (!course) return; course.level = event.target.value; touch(course); });
    $('courseLocale').addEventListener('change', event => { const course = currentCourse(); if (!course) return; course.locale = event.target.value; touch(course); });
    $('courseWorkflow').addEventListener('change', event => { const course = currentCourse(); if (!course) return; course.workflow = event.target.value === 'review' ? 'review' : 'draft'; touch(course); renderCourseEditor(); renderCatalog(); });
    $('moduleTitle').addEventListener('input', event => { const module = currentModule(), course = currentCourse(); if (!module || !course) return; module.title = text(event.target.value, 120); touch(course); renderModules(); });
    $('moduleObjective').addEventListener('input', event => { const module = currentModule(), course = currentCourse(); if (!module || !course) return; module.objective = text(event.target.value, 500); touch(course); renderPreview(); });
  }
  ...
  $('saveCourse').addEventListener('click', () => { const course = currentCourse(); if (!course) return showToast('Crea o selecciona un curso.'); course.updatedAt = now(); persist('manual'); renderAll(); showToast('Borrador guardado.'); });
  ...
  $('studioMode').textContent = core?.enabled ? 'ACADEMY CORE CONNECTED · DRAFT CACHE' : 'LOCAL DRAFT ENGINE · BACKEND READY';
  telemetry.track('content_studio_loaded', { mode: core?.enabled ? 'academy-core' : 'local-fallback', tenant: tenantId });
})();
