...
const [usersHtml, usersJs, activateHtml, activateJs, authSession, authPage, instructorJs, adapter, netlify, sw] = await Promise.all([
  read('users.html'), read('users.js'), read('activate.html'), read('activate.js'), read('auth.session.js'),
...
console.log('✓ Role separation, invitations, PBKDF2 preview accounts and clean routes validated');